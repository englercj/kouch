...
Model.key = Model.prototype.key = function (key) {
    var prefix = this.schema.get('key.prefix') || this.modelName,
        delimiter = this.schema.get('key.delimiter') || ':';
...
Model.prototype.save = function (options, cb) {
    ...
    var self = this;
    this.bucket.upsert(this.key(this._id), this._doc, options, function (err, result) {
        if (err) return cb(err);
...
Model.prototype.remove = function (options, cb) {
    ...
    var self = this;
    this.bucket.remove(this.key(this._id), this._doc, options, function (err, result) {
        if (err) return cb(err);
...
Model.prototype.model = function model (name) {
    return this.base.model(name, this.bucket.name);
};
