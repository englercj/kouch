...
Model.load = function (key, options, cb) {
    ...
    if (Array.isArray(key)) {
        return Model.loadMulti.call(this, key, cb);
    }
    else if (!key) {
        var error = new Error('Key is required to load a document.');
 ...
 * @param [callback] {Function} The function to call when the get returns.
 */
Model.loadMulti = function (keys, cb) {
    if (typeof options === 'function') {
        cb = options;
        options = null;
    }
    ...
    var self = this;
    this.bucket.getMulti(keys.map(this.key.bind(this)), function (err, results) {
...
Model.remove = function (key, options, cb) {
    ...
    if (Array.isArray(key)) {
        return Model.removeMulti.call(this, key, cb);
    }
    else if (!key) {
        var error = new Error('Key is required to remove a document.');
 ...
 * @param [callback] {Function} The function to call when the remove returns.
 */
Model.removeMulti = function (keys, cb) {
...
Model.insert = function (doc, options, cb) {
    ...
    if (Array.isArray(doc)) {
        return Model.insertMulti.call(this, key, cb);
    }
 ...
 * @param [callback] {Function} The function to call upon the completion of the operation.
 */
Model.insertMulti = function (docs, cb) {
    throw new Error('Not Yet Implemented!');
    // if (typeof options === 'function') {
    //     cb = options;
    //     options = null;
...
Model.prototype.remove = function (options, cb) {
    ...
    var self = this;
    this.bucket.remove(this.key(), options, function (err, result) {
        if (err) return cb(err);
...
Model.prototype.get = function (path, skipGetters) {
    ...
    if (obj === undefined && type && type.autoType) {
        if (!this._cache.auto) {
            this._cache.auto = {};
        }
        ...
        if (!this._cache.auto[path]) {
            ...
            if (typeof type.autoType === 'function') {
                this.set(path, obj = type.autoType.call(this), true);
            }
 ...
 * @param [options.minimize=true] {Boolean} removing undefined values and empty objects
 * @param [options.getters=true] {Boolean} run getters when getting values
 * @param [options.virtuals] {Boolean} include virtual values in the result
 * @param [options.transform] {Function} custom transform function to run
 * @return {Object} The resulting object
 */
Model.prototype.toObject = function (options) {
    options = options || {};
    options.minimize = options.minimize !== undefined ? options.minimize : true;
    options.getters = options.getters !== undefined ? options.getters : true;
