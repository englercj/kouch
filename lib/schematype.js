...
SchemaType.prototype.auto = function (type) {
    this.autoValue = type;
};
...
SchemaType.prototype.match = function (rgx, message) {
    ...
    function matchValidator (v) {
        return v != null && v !== '' ? rgx.test(v) : true;
    }
...
SchemaType.prototype.enum = function () {
    ...
    this.validators.push([this.enumValidator, errorMessage.replace('${value}', v), 'enum']);
...
SchemaType.prototype.applyValidators = function (value, fn, scope) {
    ...
    function _validate(ok, message, type, val) {
        ...
        if (ok === undefined || ok) {
            ...
            fn(err = new Error('Value at path "' + path + '" ' + message + ', got: ' + val));
        }
    }
